...
import {
  ...
  safeText,
  telemetryTokenUsage,
} from "./collaborationStorePrimitives.js";
import { installCollaborationSchema } from "./collaborationStoreSchema.js";
import {
  publicAttention as projectPublicAttention,
  publicConfigurationSession as projectPublicConfigurationSession,
  publicExecutionEvent as projectPublicExecutionEvent,
  publicMessage as projectPublicMessage,
} from "./collaborationStoreProjections.js";
import {
  buildFinalReport,
...
import {
  getDiagnostics as getDiagnosticsProjection,
  getSnapshot as getSnapshotProjection,
} from "./collaborationStoreInspection.js";
...
export class CollaborationStore {
  ...
  installSchema() {
    installCollaborationSchema(this);
  }
  ...
  publicConfigurationSession(row) { return projectPublicConfigurationSession(row); }

  publicMessage(row) { return projectPublicMessage(row); }

  publicExecutionEvent(row) { return projectPublicExecutionEvent(row); }

  publicAttention(row) { return projectPublicAttention(row); }
  ...
  getSnapshot(runId) { return getSnapshotProjection(this, runId); }

  getDiagnostics(runId) { return getDiagnosticsProjection(this, runId); }
