...
export function renderLitellmConfigYaml(provider) {
  ...
  const models = enabledProviderModelEntries(provider);
  if (models.length === 0) {
    throw new Error(`provider '${provider.name}' has no enabled model`);
  }
  ...
  for (const entry of models) {
    lines.push(`  - model_name: ${esc(`${provider.name}/${entry.id}`)}`);
    lines.push("    litellm_params:");
    lines.push(`      model: ${prefix}/${esc(entry.id)}`);
...
// Remote Share exposes an explicit allow-list of local LiteLLM providers.
// Each provider name is a stable, non-secret model alias; raw upstream model
// ids and credentials remain inside the generated loopback-only YAML.
export function renderLitellmProvidersConfigYaml(providers) {
  if (!Array.isArray(providers) || providers.length === 0) {
    throw new Error("renderLitellmProvidersConfigYaml: at least one provider is required");
  }
  const esc = (s) => String(s).replaceAll("\\", "\\\\").replaceAll('"', '\\"');
  const lines = [
    "# Generated by OriginRouter Remote Share. Do not edit by hand;",
    "# only explicitly selected local LiteLLM providers are exposed.",
    "model_list:",
  ];
  for (const provider of providers) {
    const profile = resolveProviderProfile(provider);
    const entries = remoteShareModelEntries(provider, {
      legacyRemoteEnabled: true,
    });
    if (entries.length === 0) {
      throw new Error(`provider '${provider.name}' model is required`);
    }
    const params = buildLitellmParams(provider, profile);
    for (const entry of entries) {
      lines.push(`  - model_name: ${esc(entry.provider)}`);
      lines.push("    litellm_params:");
      lines.push(`      model: ${prefixFor(profile.id)}/${esc(entry.model)}`);
      for (const litellmKey of paramsFor(profile.id)) {
        if (!params.has(litellmKey)) continue;
        lines.push(`      ${litellmKey}: "${esc(params.get(litellmKey))}"`);
      }
...
export function renderLitellmRoutesConfigYaml(allRoutes, providers) {
  ...
  // Every locally-enabled Provider model receives a stable explicit alias for
  // App chat and other OpenAI-compatible clients. Agent aliases below remain
  // independent and point at the same upstream model when configured.
  for (const [providerName, rawProvider] of Object.entries(providers || {})) {
    const provider = routeProviderForRead(rawProvider);
    if (provider.type !== "proxy" || provider.engine !== "litellm") continue;
    const profile = getLitellmProfile(provider.litellmProvider);
    const params = buildLitellmParams(provider, profile);
    for (const model of enabledProviderModelEntries(provider)) {
      const alias = `${providerName}/${model.id}`;
      if (emittedAliases.has(alias)) continue;
      emittedAliases.add(alias);
      lines.push(`  - model_name: ${esc(alias)}`);
      lines.push("    litellm_params:");
      lines.push(`      model: ${prefixFor(profile.id)}/${esc(model.id)}`);
      for (const litellmKey of paramsFor(profile.id)) {
        if (!params.has(litellmKey)) continue;
        lines.push(`      ${litellmKey}: "${esc(params.get(litellmKey))}"`);
      }
  ...
  if (!emittedAny) {
    throw new Error(
      "renderLitellmRoutesConfigYaml: no enabled local LiteLLM models or Agent routes configured.",
    );
  }
