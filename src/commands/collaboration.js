...
import {
  ...
  request,
  throwIfAborted,
} from "./collaborationTransport.js";
import {
  autoConfigurationView,
  printAutoConfiguration,
  printAttachEvent,
  printAttachSnapshot,
  printFinalReport,
  printRun,
  TERMINAL_VIEW_STATES,
} from "./collaborationOutput.js";

export { autoConfigurationView } from "./collaborationOutput.js";
...
export async function configureOnTargetCli({
  ...
  const started = await requestFn("/collaboration/local/configurations", {
    ...
    body: {
      ...
      coordinator_runtime: coordinator || "codex",
...
async function handleCollaborationCommandImpl(args, options = {}) {
  ...
  if (action === "create") {
    ...
    let run = configurationId
      ? (await request(
        `/collaboration/local/configurations/${encodeURIComponent(configurationId)}/accept`,
        { method: "POST", body: {}, signal: options.signal },
      )).run
      : (await request("/collaboration/local/runs", { method: "POST", body: payload, signal: options.signal })).run;
